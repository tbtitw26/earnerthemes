...
export const userController = {
    async topUpBalance(userId: string, amount: number, options?: { simulated?: boolean }): Promise<UserType> {
        await connectDB();

        const user = await userService.addBalance(userId, amount);

        console.log("💳 Adding balance for user:", userId);
        await transactionService.record(user._id, user.email, amount, "add", user.balance);
